...
class VectorDBManager:
    ...
    async def _wait_for_index_ready(self, index_name: str, max_wait: int = 300):
        ...
        raise TimeoutError(f"Index {index_name} not ready after {max_wait} seconds")
    ...
    async def upsert_text_embedding(self, doc_id: str, embedding: np.ndarray, 
        ...
        try:
            ...
            vector_data = embedding.tolist() if isinstance(embedding, np.ndarray) else embedding
    ...
    async def upsert_image_embeddings(self, doc_id: str, image_items: List[Dict], 
        ...
        try:
            ...
            print(f"🖼️ Upserted {len(image_items)} image embeddings for document: {doc_id}")
            return True
    ...
    async def delete_document_embeddings(self, doc_id: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                print(f"⚠️ Warning: Could not delete text embedding for {doc_id}: {e}")
            ...
            try:
                ...
                if stats.total_vector_count > 0:
                    ...
                    if image_ids_to_delete:
                        ...
                        print(f"🗑️ Deleted {len(image_ids_to_delete)} image embeddings for document: {doc_id}")
                    
            except Exception as e:
                print(f"⚠️ Warning: Could not delete image embeddings for {doc_id}: {e}")
    ...
    async def get_document_embeddings(self, doc_id: str) -> Tuple[Optional[np.ndarray], List[Dict[str, Any]]]:
        ...
        try:
            ...
            try:
                ...
                if response.matches:
                    text_embedding = np.array(response.matches[0].values, dtype=np.float32)
                    
            except Exception as e:
                print(f"⚠️ Warning: Could not retrieve text embedding for {doc_id}: {e}")
            ...
            try:
                ...
                print(f"⚠️ Warning: Could not retrieve image embeddings for {doc_id}: {e}")
